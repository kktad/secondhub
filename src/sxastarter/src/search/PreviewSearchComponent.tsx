...
import { SitecoreResponseContent, SitecoreResponseSuggestions } from './Interfaces/SitecoreResponseContent';
...
const PreviewSearchComponent = () => {
    ...
    const handleChange = async (e: any) => {
        ...
        const suggesionResponse: SitecoreResponseSuggestions[] = (response.suggestion != undefined) ? (response.suggestion.title_context_aware) : ([])
    ...
    return (
        <div className='previewContainer' ref={ref}>
            <input className='input-field' type="search" value={keyword ?? ""} onClick={handleChange} placeholder="Type to Search..." onChange={handleChange} onKeyDown={handleKeyDown} />
            {isInsideClick && (<div className="popup">
                            ...
                            suggestions?.map((suggesion, i) => {
                                return <>
                                    <Link href={{ pathname: '/searchresults', query: { q: suggesion.text } }}>
                                        <li key={i} onMouseEnter={() => handleMouseHover(suggesion.text)}>{suggesion.text}</li>
                                    </Link>
                                </>
                            })
                        }
