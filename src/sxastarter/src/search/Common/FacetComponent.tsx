...
const FacetComponent: React.FC<ChildProps> = ({ facets, onDataFromFacets }) => {
  ...
  const handleCheckboxChange = (facetid: string, facetname: string) => {
    ...
    const mainItem = [...filterPayload].find((item) => item.name === facetname);
    if (mainItem != null) {
      ...
      if (!facetItem) {
        ...
        if (mainItem?.filter.values.length != 1) {
          ...
          valueCollection.splice(
            valueCollection.findIndex((f) => f === facetid),
            1
          );
          itemsCollection.map((item) =>
            item.name === facetid
              ? { ...item, values: { ...item.filter.values, valueCollection } }
              : item
          );
          setFilterPayload(itemsCollection);
        } else {
          const itemsCollection: TypeItem[] = [...filterPayload];
          itemsCollection.splice(
            itemsCollection.findIndex((item) => item.name === facetid),
            1
          );
          setFilterPayload(itemsCollection);
        }
        ...
        return (
          <div className="facets" key={i}>
            ...
            </button>
            {
              <div
                className="checkbox"
                style={{ display: divVisibilities.includes(facet.label) ? 'block' : 'none' }}
              >
                <ul>
